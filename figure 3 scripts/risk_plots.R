...
# libraries
library(rethinking)
source("risk_functions.R")  # Load the risk function we created
...
plot_accuracy_results <- function(accuracy_results, save_plots = TRUE, 
  ...
  if (save_plots) {
    jpeg(paste0("output/figures/", "accuracy_comparison.jpg"), 
  ...
  if (save_plots) {
    jpeg(paste0("output/figures/", "risk_category_changes.jpg"), 
         units = "in", width = 8, height = 6, res = 300)
  }
