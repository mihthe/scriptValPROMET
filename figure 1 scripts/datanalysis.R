...
## use other c.dashboard function

###########################
# importing datasets.       ##when first run simulation file, dataset is 'sim'
#db <- read.csv("sim.csv")
# 
...
###########################
# converting simulated dataset 
dat <- list(
  N = nrow(sim),  ##ADD THIS LINE
  Si = sim$Si,
  Su = standardize(sim$Su),
  m_bio = standardize(sim$m_bio),
  m_surg = sim$m_sur,
  L = sim$L,
  n = as.integer(1 - sim$resp_Rx),
  y = sim$resp_Rx
)

#dat <- as.list(db)

# # BUILDING DAT_LIST FOR ULAM LATER
# dat_list <- list(
#   N = nrow(dat),             # Number of observations
#   m_surg = dat$m_surg,       # Outcome variable (vector)
#   Si = dat$Si,               # Covariate Si (vector)
#   m_bio = dat$m_bio,         # Covariate m_bio (vector)
#   n = dat$n,                 # Covariate n (vector)
#   y = dat$y,                 # Covariate y (vector)
#   Su = dat$Su,               # Covariate Su (vector)
#   L = dat$L,                 # Levels or groupings (vector)
#   L_max = max(dat$L),        # Maximum level of L
#   b_bar = -1,                # Prior for b_bar (if not a parameter)
#   e_bar = -1,                # Prior for e_bar (if not a parameter)
#   g = -1,                    # Prior for g (if not a parameter)
#   d = -1                     # Prior for d (if not a parameter)
# )
...
h_size_surf <- ulam(
  ...
  ), data=dat , chains=4 , cores=4 , iter=4000 , log_lik = TRUE) ## CHANGED DATA (SEE ALSO ABOVE)
...
lambda <- mapply(m_link , Si = sim$Si, m_bio = sim$m_bio , Su = sim$Su , L = dat$L )
...
points(1:80, log(lmu[1:80] + 0.1), pch = 16) ## extracting vector of 80 values
...
table <- matrix(unlist(coeff_table@.Data), nrow = 17) |> round(2) ## nrow s.t. to fit data length
...
newRowName <-  c("$\\beta_{L[1]}$","$\\beta_{L[2]}$","$\\beta_{L[3]}$","$\\beta_{L[4]}$","$\\beta_{L[5]}$", ## [5] added to match row.names
                 "$\\gamma_{L[1]}$","$\\gamma_{L[2]}$", "$\\gamma_{L[3]}$","$\\gamma_{L[4]}$","$\\beta_{L[5]}$", ## ... length to array extent
...
lambda <- mapply(m_link , Si = sim$Si, m_bio = sim$m_bio , Su = sim$Su , L = dat$L )
lmed <- apply( lambda , 2 , median ) ## ISSUE!!
lci <- apply( lambda , 2 , HPDI )
j <- order(lmed)
...
jpeg(paste0("output/figures/","PosPredCheck",".jpg"), 
     ...
     width = 12, height = 7, res = 300)
par(mfrow = c(1,1))
plot(NULL, xlim = c(1,10000), ylim = c(log(0.1),log(2000)), 
     bty = 'n', xaxt = 'n',  yaxt = "n", ylab = 'Mitotic count', 
     xlab = 'Cases', main = 'Posterior predictive check')
abline(v = 1:10000, lty = 1, col = scales::alpha(dat$L[j], 0.2), lwd = 10)
abline(h = log(5+ 0.1))
points(1:10000, log(sim$m_bio[j] + 0.1), col = 4, pch = 4, 
       lwd= 2, cex = 0.3 + sim$Su/23.5)
points(1:10000, log(dat$m_surg[j] + 0.1), col = 2 + dat$n[j], 
       lwd= 3, cex = 0.3 + inv_logit(dat$Si) )
segments(x0 = 1:10000, y0 = log(lci[1,j]+ 0.1), 
         y1 = log(lci[2,j]+ 0.1), lwd = 2)
points(1:10000, log(lmed[j] + 0.1), pch = 16)
axis(2, at = log(c(0.1,1,2,3,5,10,20, 50, 100)), 
     labels = c(0,1,2,3,5,10,20,50, 100), las = 2)
axis(1, at = 1:10000, labels = 1:10000)
