...
c.dashboard <- function(fit, warmup = FALSE, plot = TRUE) {
  
  # Extract CmdStanMCMC object from ulam
  if (inherits(fit, "ulam")) {
    if (!is.null(attr(fit, "cstanfit"))) {
      fit <- attr(fit, "cstanfit")
    } else if (!is.null(fit$fit)) { 
      fit <- fit$fit
    } else if (!is.null(fit@stanfit)) {
      fit <- fit@stanfit
    } else {
      stop("Cannot extract CmdStanMCMC from this ulam object")
    }
  }
  ...
  # Get posterior draws
  draws_post <- fit$draws(inc_warmup = warmup, format = "draws_array")
  
  # Get summary with proper measures
  summ <- fit$summary()
  
  # Ensure required columns exist
  if (!"ess_bulk" %in% names(summ)) {
    summ$ess_bulk <- NA_real_
  }
  ...
  sampler_df <- fit$sampler_diagnostics(format = "df", inc_warmup = warmup)
  energy <- sampler_df$energy__
  
  # Plotting
  if (plot) {
    set_nice_margins()
    par(mfrow = c(3, 1), cex.axis = 1, cex.lab = 1.2)
    
    # 1) R-hat vs ESS - with proper handling of finite values
    ess_vals <- summ$ess_bulk
    rhat_vals <- summ$rhat
    
    # Filter for finite values
    valid_idx <- is.finite(ess_vals) & is.finite(rhat_vals)
    
    if (sum(valid_idx) > 0) {
      plot(
        ess_vals[valid_idx],
        rhat_vals[valid_idx],
        xlab = "Effective sample size (bulk)",
        ylab = "R-hat",
        ylim = c(0.99, max(1.1, max(rhat_vals[valid_idx], na.rm = TRUE))),
        pch = 16, 
        col = grau()
      )
      abline(h = 1, lty = 2)
      ...
      # Add reference lines
      n_draws <- dim(draws_post)[1] * dim(draws_post)[2]  # iterations × chains
      abline(v = 0.1 * n_draws, col = "red", lty = 3)
      
      # Add legend
      legend("topright", 
             legend = c("R-hat = 1.0", "10% of total draws"),
             lty = c(2, 3), 
             col = c("black", "red"))
    } else {
      plot(1, type = "n", xlab = "", ylab = "", 
           main = "No valid ESS/Rhat data")
      text(1, 1, "Check model fitting")
    }
    
    # 2) Energy diagnostic
    if (length(energy) > 0 && any(is.finite(energy))) {
      dens(energy, adj = 0.1, xlab = "HMC energy", 
           main = "Energy Distribution")
      mu_energy <- mean(energy, na.rm = TRUE)
      sd_energy <- sd(energy, na.rm = TRUE)
      curve(dnorm(x, mu_energy, sd_energy), 
            add = TRUE, col = rangi2, lwd = 2)
      
      legend("topright",
             legend = c("Observed", "Expected Normal"),
             col = c("black", rangi2),
             lty = 1,
             lwd = c(1, 2))
    } else {
      plot(1, type = "n", main = "No energy data available")
    }
    
    # 3) Rank histogram using bayesplot (correct package!)
    draws_for_rank <- fit$draws(format = "draws_array", inc_warmup = FALSE)
    
    if ("lp__" %in% dimnames(draws_for_rank)$variable) {
      # Use bayesplot's mcmc_rank_hist (NOT posterior::rank_hist)
      print(bayesplot::mcmc_rank_hist(draws_for_rank, pars = "lp__"))
    } else {
      plot(1, type = "n", main = "No lp__ available")
      text(1, 1, "Cannot create rank histogram")
    }
  }
  
  # Return summary invisibly
  invisible(list(summary = summ, energy = energy))
}
